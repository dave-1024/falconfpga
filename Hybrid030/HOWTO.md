...
The card contract is from `hybrid_musashi/falcon_m28.c` (“the card is the machine”).
...
| Line | Meaning |
|---|---|
| `ROM=` | TOS image loaded to the guest ROM window at `0x00E00000`. You supply the file. 512K TOS 4.04 is what this bench used as the default guest. |
| `DISKA=` | Floppy A: `.ST` image. Writes go through to the card if the file is contiguous. |
| `DISKB=` | Floppy B: same idea. |

Any missing line or missing file falls back: embedded EmuTOS, blank A:, blank B:. A missing card does not brick the firmware.
...
1. Format FAT32.
2. Copy a 512K TOS or EmuTOS image, e.g. `TOS404.IMG` or `EMUTOS.IMG`.
3. Copy a small `.ST` as `DISKA.ST` if you want a writable A:.
4. Create `FALCON.CFG` in the root:
...
- Not pin-level SPI documentation. The driver is bit-bang on the REV10 TF-slot GPIOs described in `hybrid_musashi/falcon_m28.c`.
