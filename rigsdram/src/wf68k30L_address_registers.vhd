...
begin
    ...
    begin
        ...
        if RESET = '1' or UNMARK = '1' then
            AR_USED_1(3) <= '0';
            AR_USED_2(3) <= '0';
            -- [COLD] 68030 reset SR is S=1 M=0.  MSBIT had no reset and
            -- only updated on AR_MARK_USED, so move.l %sp could write
            -- USP/MSP while exception stacking still used ISP.
            MSBIT <= "01";
    ...
    variable PCBASE             : std_logic_vector(31 downto 0); -- F19/U2: suppressible PC base.
    begin
        I_S_IS := I_S & I_IS;        
        ...
        -- F16/H23: base suppression was gated on FETCH_MEM_ADR, true only in
        -- FETCH_STATE = FETCH_MEMADR (control.vhd:492), a state entered ONLY
        -- when the extension word requests memory indirection (:1898/:1961).
        -- For a plain full-format EA with I/IS=000 the BS bit was decoded and
        -- then never acted on, so the base register was added regardless.
        -- Measured: A0=$9100 + bd=$9120 + Xn*2=$10 = $12230, where the manual
        -- requires $9130. The B_S and F_E terms already confine this to the
        -- suppressed case, so unsuppressed paths are untouched.
        --
        -- CORRECTION (F19/H27): this arm covers the An path ONLY. The second
        -- arm below, for ADR_MODE="111" / AMODE_SEL="011", is DEAD CODE -- the
        -- PC-relative EA is computed from PCVAR and NEVER READS ADR_MUX, so
        -- zeroing ADR_MUX there has no effect and never did, before or after
        -- F16. It is retained only because removing it is an unvectored change
        -- to shipped, silicon-confirmed code. PC-relative base suppression is
        -- handled by PCBASE in the "111" arm; see F19.
        if ADR_MODE = "110" and F_E = '1' and B_S = '1' then
            ADR_MUX := x"00000000"; -- Base register suppress (An path).
        elsif ADR_MODE = "111" and AMODE_SEL = "011" and F_E = '1' and B_S = '1' then
            ADR_MUX := x"00000000"; -- DEAD CODE: PC path uses PCBASE, not ADR_MUX.
        ...
        case ADR_MODE is
            ...
            when "110" =>
                if F_E = '0' then -- Brief extension word.
                    ...
                    case I_S_IS is
                        ...
                        -- F17/H24: "1000" is I_S = 1, the INDEX SUPPRESSED
                        -- full-format case with no memory indirection. It
                        -- shared the arm above and so added INDEX_SCALED
                        -- anyway. Measured: A0=$9100 + bd=4 + Xn*1=8 gave
                        -- $910C where the manual requires $9104. The suppressed
                        -- index INDIRECT variants ("1001"/"1010"/"1011") were
                        -- already correct below, which is why only the plain
                        -- case failed.
                        when "1000" => -- Index suppressed, no memory indirect.
                            ADR_EFF_VAR := ADR_MUX + BASE_DISPL; -- (bd, An).
            ...
            when "111" =>
                case AMODE_SEL is
                    ...
                    when "011" =>
                        -- F19/U2: the PC arm computes from PCVAR directly and
                        -- never reads ADR_MUX, so F16's ADR_MUX zeroing for
                        -- AMODE_SEL="011" was DEAD CODE and BS had no effect on
                        -- PC-relative forms. PCBASE is the suppressible base.
                        -- Measured: BS=1 with bd=$9120, Xn*2=$10 read $00000000
                        -- instead of the $9130 marker.
                        ...
                        if F_E = '0' then -- Brief extension word.
                            ...
                            case I_S_IS is
                                when "0000" => -- No memory indirect action.
                                    ADR_EFF_VAR := PCBASE + BASE_DISPL + INDEX_SCALED; -- (bd, PC, Xn, SIZE*SCALE).
                                -- F20/U1: "1000" is IS=1, index suppressed. It
                                -- shared the arm above and added INDEX_SCALED
                                -- anyway -- the same defect F17 fixed on the An
                                -- path, left behind on the PC path. Measured:
                                -- the read landed 8 bytes on, exactly d2*scale.
                                when "1000" => -- Index suppressed, no memory indirect.
                                    ADR_EFF_VAR := PCBASE + BASE_DISPL; -- (bd, PC).
                                when "0001" | "0010" | "0011" => -- Memory indirect preindexed.
                                    if FETCH_MEM_ADR = '1' then
                                        ADR_EFF_VAR := PCBASE + BASE_DISPL + INDEX_SCALED;
                                    else
                                        ADR_EFF_VAR := MEM_ADR + OUTER_DISPL;
                                    end if;
                                when "0101" | "0110" | "0111" => -- Memory indirect postindexed.
                                    if FETCH_MEM_ADR = '1' then
                                        ADR_EFF_VAR := PCBASE + BASE_DISPL;
                                    else
                                        ADR_EFF_VAR := MEM_ADR + INDEX_SCALED + OUTER_DISPL;
                                    end if;
                                when "1001" | "1010" | "1011" => -- Memory indirect.
    ...
    begin
        ...
        if (ISP_DEC = '1' and MBIT = '1') or (AR_DEC = '1' and AR_PNTR_1 = 7 and SBIT = '1' and MBIT = '1') then -- F8/H14: master-stack build decrement.
            case OP_SIZE is
        ...
        ---------------------------------------- ISP section ----------------------------------------
        -- [COLD] One chain.  Four separate ifs all wrote ISP_REG;
        -- GHDL last-assignment-wins, Gowin need not.  ISP_WR (reset
        -- SSP fetch) is highest so RESTORE_ISP cannot lose to RESET
        -- or a same-cycle DEC.
        if ISP_WR = '1' then
            ISP_REG <= AR_IN_1;
            ...
            ISP_REG <= (others => '0');
        elsif AR_INC = '1' and AR_PNTR_1 = 7 and SBIT = '1' and MBIT = '0' then
            case OP_SIZE is
                when BYTE       => ISP_REG <= ISP_REG + "10";
                when WORD       => ISP_REG <= ISP_REG + "10";
                when others     => ISP_REG <= ISP_REG + "100";
            end case;
        elsif (ISP_DEC = '1' and MBIT = '0') or (AR_DEC = '1' and AR_PNTR_1 = 7 and SBIT = '1' and MBIT = '0') then
            case OP_SIZE is
                when BYTE       => ISP_REG <= ISP_REG - "10";
                when WORD       => ISP_REG <= ISP_REG - "10";
                when others     => ISP_REG <= ISP_REG - "100";
            ...
            ISP_REG <= ISP_REG + DISPLACEMENT + "100";
