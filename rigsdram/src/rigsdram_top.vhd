...
entity RIGSDRAM_TOP is
    generic(
        ...
        AUTO_WARM  : integer := 1;   -- [PRODUCT] auto warm-reset pulse after
                                     -- first release. Warm is a healthy
                                     -- machine (see warmboot.log: $7F00
                                     -- write, H=1, 16 cycles of real code).
                                     -- This makes power-on do what S1 does.
        AW_FREE    : integer := 256
    );
    port(
...
begin
    ...
    -- the PLL locks).  sys_rst_n additionally holds until pll_lock, so
    -- the CPU and controller never see a pre-lock or unlocked clock --
    -- which, given how much of this bring-up turned on power-up state,
    ...
    sys_rst_n <= por_n and rstn_sync(2) and pll_lock;
    ...
    begin
        if rising_edge(core_clk) then
            if sys_rst_n = '0' then
                ...
                -- [F53] Hold the CPU in reset for the WHOLE PLL acquisition.
                cpu_rst_n  <= '0';
                cpu_halt_n <= '0';
                brk_cnt    <= (others => '0');
                aw_st      <= AW_WAIT;
    ...
    U_ADP: entity work.sdram_bus_adapter
        port map(clk => clk50, rst_n => sys_rst_n,   -- [F43] was CLK
    ...
    begin
        if rising_edge(clk50) then
            if sys_rst_n = '0' then init_done <= '0';
